...
# Script (actually running the function)

DATA_DIR <- './data'
R_DIR <- './R'

source(file.path(R_DIR, 'add_deployment_id.R'))

add_deployment_id_output <- add_deployment_id(
  read.csv(file.path(DATA_DIR, 'input_add_deployment_id.csv'))[1:10],
  read.csv(file.path(DATA_DIR, 'input_reference.csv'))
)
