...
data_processing <- function(){
  ...
  str(list_rip)
  return(list(
    homerange_groupsize_df = d_hr_gs,
    riparian_list = list_rip
  ))

}
# 2. Modeling Code ----

data_modeling <- function(d_hr_gs, list_rip) {
...
data_modeling(
  processed_data[['homerange_groupsize_df']],
  processed_data[['riparian_list']]
)
